<div>
    <div class="mb-6 flex justify-between items-center">
        <h2 class="text-2xl font-semibold text-gray-800">Listado de Docentes</h2>
        <div class="flex space-x-2">
            <a href="{{ route('docentes.registro') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 active:bg-indigo-800 focus:outline-none focus:border-indigo-900 focus:ring ring-indigo-300 disabled:opacity-25 transition">
                Agregar Nuevo
            </a>
                    ...
                    @forelse ($docentes as $docente)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex space-x-2">
                                    <a href="{{ route('docentes.ver', $docente->id) }}" class="text-indigo-600 hover:text-indigo-900">
                                        Ver
                                    </a>
                                    <a href="{{ route('docentes.editar', $docente->id) }}" class="text-blue-600 hover:text-blue-900">
                                        Editar
                                    </a>
                                    <button onclick="showDeleteConfirmation({{ $docente->id }})" class="text-red-600 hover:text-red-900">
                                        Eliminar
                                    </button>
                                </div>
                            </td>
    ...
    <div class="mt-4 flex items-center justify-between">
        <div class="text-sm text-gray-700">
            Mostrando <span class="font-medium">{{ $docentes->firstItem() }}</span> a 
            <span class="font-medium">{{ $docentes->lastItem() }}</span> de 
            <span class="font-medium">{{ $docentes->total() }}</span> resultados
        </div>
        <div class="flex space-x-2">
            {{ $docentes->links() }}
        </div>
    ...
    <script>
        window.addEventListener('show-delete-confirmation', (event) => {
            ...
            if (confirm('¿Está seguro de que desea eliminar este docente? Esta acción no se puede deshacer.')) {
                @this.call('eliminarDocente', id);
            }
        });
    </script>
</div>